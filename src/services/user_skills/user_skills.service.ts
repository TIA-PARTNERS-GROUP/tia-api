...
} from "types/user_skills/user_skills.validation.js";
...
export class UserSkillsService {
  ...
  /**
   * Get all users who possess a specific skill.
   * @param skillId The unique identifier of the skill.
   * @returns A promise resolving to an array of user profiles.
   */
  public async getUsersBySkill(skillId: number): Promise<UserResponse[]> {
    const skill = await prisma.skills.findUnique({
      where: { id: skillId },
    });
    ...
    const relationships = await prisma.user_skills.findMany({
    ...
    return relationships.map((relationship) => relationship.users);
  }
