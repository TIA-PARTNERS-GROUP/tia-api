...
const mapToBusinessConnectionResponse = (
...
export class BusinessConnectionService {
  ...
  public async initiateConnection(
    data: InitiateConnectionInput,
  ): Promise<BusinessConnectionResponse> {
    try {
      const connection = await prisma.business_connections.create({
        data: {
          ...data,
          status: "pending",
        },
      ...
      if (error.code === "P2002") {
        throw new ApiError(
          ...
          "A connection of this type already exists between these two businesses.",
        );
      }
      if (error.code === "P2003") {
        throw new ApiError(
          StatusCodes.NOT_FOUND,
          "One or both business IDs or the initiating user ID is invalid.",
        );
      }
      throw new ApiError(
        StatusCodes.INTERNAL_SERVER_ERROR,
        "Failed to initiate business connection.",
      );
    }
  }
  ...
  public async getConnectionById(
    connectionId: number,
  ): Promise<BusinessConnectionResponse | null> {
    const connection = await prisma.business_connections.findUnique({
  ...
  public async updateConnectionStatus(
    ...
    try {
      const updatedConnection = await prisma.business_connections.update({
        where: { id: connectionId },
        data: data,
        include: {
          businesses_business_connections_initiating_business_idTobusinesses: true,
  ...
  public async getConnectionsForBusiness(
    ...
    try {
      const connections = await prisma.business_connections.findMany({
        where: {
          OR: [
      ...
      return connections as ConnectionSummaryResponse[];
    } catch (error) {
      throw new ApiError(
        StatusCodes.INTERNAL_SERVER_ERROR,
        "Failed to retrieve business connections.",
      );
    }
  }
}
