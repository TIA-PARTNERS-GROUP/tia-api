...
func TestBusinessConnectionService_Integration_CreateAndGet(t *testing.T) {
	cleanupTestDB(t)
	businessConnectionService := services.NewBusinessConnectionService(testDB)
	...
	result = testDB.Create(&business1)
	assert.NoError(t, result.Error)
	...
	result = testDB.Create(&business2)
	assert.NoError(t, result.Error)
