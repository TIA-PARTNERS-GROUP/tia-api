...
func TestEventService_Integration_CreateAndGet(t *testing.T) {
	cleanupTestDB(t)
	eventService := services.NewEventService(testDB)
...
func TestEventService_Integration_GetEventsFiltered(t *testing.T) {
	...
	testDB.Create(&models.Event{EventType: "login", UserID: &user1.ID, Payload: datatypes.JSON(`{}`)})
	testDB.Create(&models.Event{EventType: "login", UserID: &user2.ID, Payload: datatypes.JSON(`{}`)})
	testDB.Create(&models.Event{EventType: "logout", UserID: &user1.ID, Payload: datatypes.JSON(`{}`)})
	testDB.Create(&models.Event{EventType: "system_error", UserID: nil, Payload: datatypes.JSON(`{}`)})
