...
func TestDailyActivityEnrolmentService_Integration(t *testing.T) {
	cleanupTestDB(t)
	enrolmentService := services.NewDailyActivityEnrolmentService(testDB)
	...
	testDB.Create(&models.DailyActivityEnrolment{UserID: user2.ID, DailyActivityID: activity1.ID})
	testDB.Create(&models.DailyActivityEnrolment{UserID: user1.ID, DailyActivityID: activity2.ID})
